...
fn read_com_cfg(arg_port: Option<String>, arg_baud: Option<u32>) -> (String, u32) {
    if let Some(p) = arg_port {
        return (normalize_win_com(&p), arg_baud.unwrap_or(9600));
    }
    let env_port = std::env::var("ACC_PRINTER_COM").ok();
    let env_baud = std::env::var("ACC_PRINTER_BAUD").ok().and_then(|s| s.parse().ok());
    if let Some(p) = env_port {
        return (normalize_win_com(&p), env_baud.unwrap_or(9600));
    }
    #[cfg(windows)]
    {
        let keywords = [
            "ncr", "epson", "star", "bixolon", "rongta", "xprinter", "pos", "receipt", "thermal",
        ];
        if let Ok(ports) = serialport::available_ports() {
            if let Some(best) = ports.into_iter().find(|p| {
                let n = p.port_name.to_lowercase();
                keywords.iter().any(|k| n.contains(k))
            }) {
                return (normalize_win_com(&best.port_name), env_baud.unwrap_or(9600));
            }
        }
    }
    (normalize_win_com("COM7"), 9600)
}

#[tauri::command]
async fn escpos_print_text_ar_custom_serial(
    port: Option<String>,
    baud: Option<u32>,
    codepage: u8,
    contextual: Option<u8>,
    ...
    let mut p =
        Printer::new(driver, Protocol::default(), Some(PrinterOptions::default())).debug_mode(Some(DebugMode::Hex));
    let p = p.init().map_err(|e| e.to_string())?; // single reset

    let mut cmd: Vec<u8> = vec![0x1B, 0x74, codepage];
    if let Some(m) = contextual {
        cmd.extend_from_slice(&[0x1C, 0x43, m]);
    }
    cmd.extend_from_slice(&[0x1B, 0x61, 0x01]);
    cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("متجر عينة")));
    cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("123 شارع الرئيسي")));
    cmd.extend_from_slice(b"\n\n");
    cmd.extend_from_slice(b"================================\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("الأصناف")));
    cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(b"================================\n\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("تفاح")));
    cmd.extend_from_slice(b"\n  2x @ 2.50 EGP = 5.00 EGP\n\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("موز")));
    cmd.extend_from_slice(b"\n  3x @ 1.50 EGP = 4.50 EGP\n\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("برتقال")));
    cmd.extend_from_slice(b"\n  1x @ 3.00 EGP = 3.00 EGP\n\n");
    cmd.extend_from_slice(b"================================\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("المجموع الفرعي: 7.00 ج.م")));
    cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("الضريبة (10٪): 0.70 ج.م")));
    cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(b"================================\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("الإجمالي: 7.70 ج.م")));
    cmd.extend_from_slice(b"\n================================\n\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("شكراً لك على الشراء!")));
    cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("نتمنى رؤيتك مرة أخرى")));
    cmd.extend_from_slice(b"\n\n\n\n");
    cmd.extend_from_slice(&[0x1D, 0x56, 0x00]); // cut

    let res: ER<()> = p.custom(&cmd).and_then(|p| p.print()).map(|_| ());
    res.map(|_| format!(
        "✅ Serial OK: {} @{} | CP={} ctx={:?}",
        port_final, baud_final, codepage, contextual
    ))
...
// --- Helpers for serial printing and simple RTL visual aid ---
// NOTE: This is a naive visual reversal to make quick tests more legible on devices without shaping.
fn rtl_visual(s: &str) -> String {
    s.chars().rev().collect::<String>()
}
...
// Serial (COM) variant of the custom Arabic test using escpos-rs SerialDriver
#[tauri::command]
async fn escpos_print_text_ar_custom_serial(
    port: Option<String>,
    baud: Option<u32>,
    ...
    use escpos::{driver::SerialDriver, printer::Printer, printer_options::PrinterOptions, utils::*, errors::Result as ER};
    ...
    // Single reset via .init()
    let p = p.init().map_err(|e| e.to_string())?;

    // Build ESC/POS block: set code page, optional contextual, sample text, cut
    let mut cmd: Vec<u8> = vec![0x1B, 0x74, codepage];
    if let Some(m) = contextual { cmd.extend_from_slice(&[0x1C, 0x43, m]); }
    ...
    cmd.extend_from_slice(b"================================\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("الأصناف"))); cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(b"================================\n\n");

    // Items (keep numeric details LTR for clarity)
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("تفاح"))); cmd.extend_from_slice(b"\n  2x @ 2.50 EGP = 5.00 EGP\n\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("موز"))); cmd.extend_from_slice(b"\n  3x @ 1.50 EGP = 4.50 EGP\n\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("برتقال"))); cmd.extend_from_slice(b"\n  1x @ 3.00 EGP = 3.00 EGP\n\n");

    cmd.extend_from_slice(b"================================\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("المجموع الفرعي: 7.00 ج.م"))); cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("الضريبة (10٪): 0.70 ج.م"))); cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(b"================================\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("الإجمالي: 7.70 ج.م"))); cmd.extend_from_slice(b"\n================================\n\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("شكراً لك على الشراء!"))); cmd.extend_from_slice(b"\n");
    cmd.extend_from_slice(&encode_windows_1256(&rtl_visual("نتمنى رؤيتك مرة أخرى"))); cmd.extend_from_slice(b"\n\n\n\n");

    // Cut
    cmd.extend_from_slice(&[0x1D, 0x56, 0x00]);

    let res: ER<()> = p.custom(&cmd).and_then(|p| p.print()).map(|_| ());
    res.map(|_| format!("✅ Serial OK: {} @{} | CP={} ctx={:?}", port_final, baud_final, codepage, contextual))
        .map_err(|e| format!("Serial print failed: {}", e))
}
